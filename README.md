...
**開放性目標更有效** — Infinity 最適合「沒有固定終點、每輪都能接續上一輪成果」的工作，例如：

- 不斷撰寫小說或文章續集（接續 `draft.md` 寫下一段）
- 不斷爬取、整理、彙總網路內容
- 不斷模擬、探索、產生變體或測試案例
- 長期維護的程式重構、文件補全、測試修補

這類任務沒有單一「完成答案」，每一輪都能在前一輪基礎上推進，越 loop 越有累積。

相對地，**單一目標、可一次算完**的工作（例如固定輸入的數學計算、單次腳本執行、明確的 build/deploy 一步到位）反覆 loop **效益不大** — AI 容易重複同樣結果。此時可設定 `execution.max_rounds` 限制輪次，跑完即停；或只在需要重試、分批處理時使用。
...
- **控制台** — 選設定檔、啟動/停止、工作目錄覆寫、即時日誌、環境診斷
...
控制台會即時顯示啟動時將使用的路徑；執行中可在分頁列右側查看輪次與耗時。
...
- Textual TUI：控制台 + 設定編輯器 + AI 生成提示詞 + 分頁列即時狀態
